  ...
  return (
    <div className="w-full flex flex-col gap-6">
      {results.map((item, index) => (
        <SearchItem key={index} categories={item.categories} name={item.name} description={item.description} image={item.image} price={item.price} sale={item.sale} stars={item.stars} reviews={item.reviews} duration={item.duration} ranks={item.ranks} slug={item.slug} />
